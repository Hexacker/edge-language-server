...
export class EdgeValidator {
    ...
    this.walkTree(tree.rootNode, (node) => {
      ...
      switch (node.type) {
        ...
        case 'forelse_directive':
          blockStack.push({ type: 'forelse', node });
          this.validateForelseDirective(node, document, diagnostics);
          break;
        ...
        case 'block_directive':
          ...
          break;
        case 'for_directive':
          blockStack.push({ type: 'for', node });
          this.validateForDirective(node, document, diagnostics);
          break;
        case 'while_directive':
          blockStack.push({ type: 'while', node });
          this.validateWhileDirective(node, document, diagnostics);
          break;
        case 'verbatim_directive':
          blockStack.push({ type: 'verbatim', node });
          this.validateVerbatimDirective(node, document, diagnostics);
          break;
        case 'endphp_directive':
          blockStack.push({ type: 'endphp', node });
          this.validateEndPhpDirective(node, document, diagnostics);
          break;
      }

      // Check for @end* directives without matching block
      if (node.type.startsWith('end') && node.type !== 'end_directive') {
        const directiveType = node.type.replace('end', '').replace('_directive', '');
        let found = false;
      ...
      // Validate includes
      if (node.type === 'include_directive' || node.type === 'include_if_directive') {
  ...
  private validateUnlessDirective(node: any, document: TextDocument, diagnostics: Diagnostic[]) {
  ...
  private validateForelseDirective(node: any, document: TextDocument, diagnostics: Diagnostic[]) {
  ...
  private validateSlotDirective(node: any, document: TextDocument, diagnostics: Diagnostic[]) {
  ...
  private validateSectionDirective(node: any, document: TextDocument, diagnostics: Diagnostic[]) {
  ...
  private validateBlockDirective(node: any, document: TextDocument, diagnostics: Diagnostic[]) {
  ...
  private validateForDirective(node: any, document: TextDocument, diagnostics: Diagnostic[]) {
    // Validate @for syntax
    const text = node.text;
  ...
  private validateWhileDirective(node: any, document: TextDocument, diagnostics: Diagnostic[]) {
  ...
  private validateVerbatimDirective(node: any, document: TextDocument, diagnostics: Diagnostic[]) {
    // @verbatim doesn't need special validation
  }

  private validateEndPhpDirective(node: any, document: TextDocument, diagnostics: Diagnostic[]) {
